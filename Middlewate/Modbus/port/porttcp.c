...
/* ----------------------- Static variables ---------------------------------*/
static struct tcp_pcb *pxPCBListen = NULL;
static struct tcp_pcb *pxPCBClient = NULL;
...
extern void prvvMBTCPLogFrame( UCHAR * pucMsg, UCHAR * pucFrame, USHORT usFrameLen );
...
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    ...
    if( pxPCBClient )
    {
        ...
        if( tcp_write( pxPCBClient, pucMBTCPFrame, ( u16_t ) usTCPLength, 0x01 ) == ERR_OK )
        {
#ifdef MB_TCP_DEBUG
